...
<h3><?php echo JText::_('J2STORE_ORDER_STATISTICS');?></h3>
	...
	<tbody>
		<tr>
			...
			<td>
			<?php
				echo $order_model->clearState()->nozero(0)->getOrdersTotal();
			?>
			</td>
			<td>
			<?php
				echo J2StorePrices::number($order_model->clearState()->nozero(0)->moneysum(1)->getOrdersTotal());
			?>
			</td>
		</tr>

		<tr>
			<td><?php echo JText::_('J2STORE_TOTAL_CONFIRMED_ORDERS'); ?></td>
			<td>
			<?php
				echo $order_model->clearState()->paystate(1)->nozero(1)->getOrdersTotal();
			?>
			</td>
			<td>
			<?php
		...
		<tr>
			...
			<?php
						...
						if($y < 2011) $y = 2011;
						$daysin = gmdate('d');
						$numsubs = $order_model->clearState()
							->since(gmdate('Y').'-'.gmdate('m').'-01')
							->until(gmdate('Y').'-'.gmdate('m').'-'.$lmday.' 23:59:59')
							->nozero(1)
							->paystate(1)
							->getOrdersTotal();
						$summoney = $order_model->clearState()
							->since(gmdate('Y').'-'.gmdate('m').'-01')
							->until(gmdate('Y').'-'.gmdate('m').'-'.$lmday.' 23:59:59')
							->moneysum(1)
							->paystate(1)

...
?>
<div class="j2store">
<form action="index.php?option=com_j2store&view=shippingmethods"
	...
	<table class="adminlist table table-striped">
		<thead>
			<tr>
				...
				<th class="title"><?php echo JHTML::_('grid.sort',  'J2STORE_SHIPM_ID', 'a.id', $this->lists['order_Dir'], $this->lists['order'] ); ?>
				</th>
				<th class="title"><?php echo JHTML::_('grid.sort',  'J2STORE_SHIPM_NAME', 'shipping_method_name', $this->lists['order_Dir'], $this->lists['order'] ); ?>
				</th>
				<th width="5%" nowrap="nowrap"><?php echo JHTML::_('grid.sort',  'J2STORE_SHIPM_STATE', 'a.published', $this->lists['order_Dir'], $this->lists['order'] ); ?>
				</th>
		...
		<tbody>
			...
			for ($i=0, $n=count( $this->items ); $i < $n; $i++)
			{
				...
				$link 	= JRoute::_( 'index.php?option=com_j2store&view=shippingmethods&task=edit&cid[]='. $row->id );
			...
			<tr class="<?php echo "row$k"; ?>">
				...
				<td><?php echo $this->escape($row->id); ?>
				</td>
				<td><span class="editlinktip hasTip"
					title="<?php echo JText::_( 'J2STORE_SHIPM_VIEW_SHIPPING_METHOD' );?>::<?php echo $this->escape($row->shipping_method_name); ?>">
						<a href="<?php echo $link ?>"> <?php echo $this->escape($row->shipping_method_name); ?>
					</a>
				</span> <br /> <b><?php echo JText::_('J2STORE_SHIPM_TYPE')?> </b>:
					<?php
					$model = $this->getModel('shippingmethods');
					$row->shipping_method_type = $model->getShippingMethodType($row->shipping_method_type);
					echo $this->escape($row->shipping_method_type); ?> <span
					style="float: right"> [<?php echo J2StorePopup::popup( "index.php?option=com_j2store&view=shippingmethods&task=setrates&id=".$row->id."&tmpl=component", JText::_( "J2STORE_SHIPM_SET_RATES" ) ); ?>]
				</span>
				</td>

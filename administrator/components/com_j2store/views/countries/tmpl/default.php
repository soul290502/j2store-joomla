...
<form action="<?php echo $action;?>" name="adminForm" class="adminForm"
	...
	<table class="table">
		<tr>
			<td>
				<!-- search filter --> <label for="filter_search"> <?php echo JText::_('J2STORE_FILTER_SEARCH');?>
	...
	<table id="adminTable" class="adminlist table table-striped">
		...
		<thead>
			<th width="1px"><?php echo JText::_('J2STORE_NUM'); ?></th>
			<th width="1px"><input type="checkbox" name="checkall-toggle"
				value="" title="<?php echo JText::_('JGLOBAL_CHECK_ALL'); ?>"
				onclick="Joomla.checkAll(this)" />
			</th>

			<th class="name"><?php echo JHtml::_('grid.sort',  'J2STORE_COUNTRY_NAME', 'a.country_name', $listDirn, $listOrder); ?>
			</th>
			<th class="code2"><?php echo JHtml::_('grid.sort',  'J2STORE_COUNTRY_CODE2', 'a.country_isocode_2', $listDirn, $listOrder); ?>
			</th>
			<th class="code3"><?php echo JHtml::_('grid.sort',  'J2STORE_COUNTRY_CODE3', 'a.country_isocode_3', $listDirn, $listOrder); ?>
			</th>
			<th width="5%"><?php echo JHtml::_('grid.sort',  'JPUBLISHED', 'a.state', $listDirn, $listOrder); ?>
			</th>
			<th class="id"><?php echo JHtml::_('grid.sort',  'J2STORE_COUNTRY_ID', 'a.country_id', $listDirn, $listOrder); ?>
			</th>
		...
		<tbody>
			...
			<tr class="row<?php echo $i%2; ?>">
				<td><?php echo $i+1; ?></td>
				<td><?php echo JHtml::_('grid.id',$i,$item->country_id); ?>
				</td>
				<td><?php echo $this->escape($item->country_name);?>
				</td>
				<td><?php echo $this->escape($item->country_isocode_2 );?></td>
				<td><?php echo $this->escape($item->country_isocode_3 );?></td>
				<td class="center"><?php echo JHtml::_('jgrid.published', $item->state, $i, 'countries.', $canChange, 'cb'); ?>
				</td>

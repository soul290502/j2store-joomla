...
class J2StoreModelGeoZones extends JModelList {
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		parent::populateState('a.geozone_id', 'asc');
	}
	...
	protected function getListQuery()
	{
		...
		$query->select(
				$this->getState(
						'list.select',
						'a.geozone_id, a.geozone_name,a.state,a.ordering'
				)
		);
		...
		if (!empty($search)) {
			if (stripos($search, 'id:') === 0) {
				$query->where('a.geozone_id = '.(int) substr($search, 3));
			} else {
				$search = $db->Quote('%'.$db->escape($search, true).'%');
				$query->where('(a.geozone_name LIKE '.$search.')');
			}
		}
		...
		if($orderCol == 'a.geozone_id' ) {
			$orderCol = 'a.geozone_id '.$orderDirn.', a.geozone_id';
		} else {
			$orderCol = 'a.geozone_id '.$orderDirn.', a.geozone_id';
		}

 ...
 * @since 2.5
 */
class J2StoreModelMigrate extends J2StoreModel
{
	...
	function getAttributes() {
		...
		$query->select('pa.*');
		$query->from('#__j2store_productattributes AS pa');
		$query->select('p.title');
		$query->join('LEFT', '#__content AS p ON pa.product_id=p.id');
		$db->setQuery($query);
		try {
    		$rows = $db->loadObjectList();

		} catch (Exception $e) {
    		$this->setError(JText::_('J2STORE_MIGRATE_CURRENT_VERSION'));
    	}
		return $rows;
	...
	function getPAOWithEqualSign($attribute_id) {
		...
		$query = $db->getQuery(true);
		$query->select('*');
		$query->from('#__j2store_productattributeoptions');
	...
	function migrate() {
		...
		if(!self::canMigrate() || J2STORE_ATTRIBUTES_MIGRATED==1) {
			$this->setError(JText::_('J2STORE_NO_ATTRIBUTES_TO_MIGRATE'));
			return false;
		}
		...
		if(JFile::exists($file)) {
			...
			$buffer = "define('J2STORE_ATTRIBUTES_MIGRATED', '1');";
			fwrite($fh, $buffer);
			fclose($fh);
		}

		$this->setError(JText::_('J2STORE_MIGRATE_SUCCESSFUL'));
		return true;
	} else {
		$this->setError(JText::_('J2STORE_NO_ATTRIBUTES_TO_MIGRATE'));
		return false;
	...
	function getProductAttributeOptions($attribute_id) {
		...
		$db = JFactory::getDbo();
		$query = 'SELECT * FROM #__j2store_productattributeoptions WHERE productattribute_id='.$attribute_id;
		$db->setQuery($query);
		$rows = $db->loadObjectList();
		return $rows;

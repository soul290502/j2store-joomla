 ...
 * @since 2.5
*/
class J2StoreModelTaxProfile extends JModelAdmin
{
	...
	public function getForm($data = array(), $loadData = true)
	{
		...
		// Get the form.
		$form = $this->loadForm('com_j2store.taxprofile', 'taxprofile', array('control' => 'jform', 'load_data' => $loadData));
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_j2store.edit.taxprofile.data', array());
	...
	public function saveTaxRule(){
		...
		$query->select('COUNT(*)');
		$query->from('#__j2store_taxrules AS r');
		...
		if($result==1){
			$this->setError(JText::_('J2STORE_TAXRULE_ALREADY_EXISTS'));
			return false;
		}
	...
	public function getTaxRules (){
		...
		$db		= $this->getDbo();
		$query	= $db->getQuery(true);
		$query->select('r.taxrule_id as id,b.taxrate_id, b.taxrate_name,b.tax_percent, r.address');
		$query->from('#__j2store_taxrules AS r');
		$query->join('LEFT','#__j2store_taxrates AS b ON b.taxrate_id=r.taxrate_id');
	...
	public function getTaxRateList($default_val=''){
		...
		$taxrate_options[] = JHTML::_('select.option', '', JText::_('J2STORE_SELECT_TAXRATE'));
	...
	public function getAddressList($default_val=''){
		...
		$address_options[] = JHTML::_('select.option', '', JText::_('J2STORE_SELECT_ADDRESS'));
		$address_options[] =  JHTML::_('select.option', 'shipping', JText::_('J2STORE_SHIPPING_ADDRESS'));
		$address_options[] =  JHTML::_('select.option', 'billing', JText::_('J2STORE_BILLING_ADDRESS'));
		$address_options[] =  JHTML::_('select.option', 'store', JText::_('J2STORE_STORE_ADDRESS'));

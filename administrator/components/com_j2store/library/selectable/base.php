...
class J2StoreSelectableBase {
	...
	function getFormatedCustomFields($row, $layout='customfields', $type='billing') {
		...
		$templatePath = JPATH_SITE.'/templates/'.$app->getTemplate().'/html/com_j2store/orders/'.$layout.'.php';
		$defaultPath = JPATH_SITE.'/components/com_j2store/views/orders/tmpl/'.$layout.'.php';
	...
	function getField($fieldid,$type=''){
		if(is_numeric($fieldid)){
			$query = 'SELECT * FROM #__j2store_field WHERE field_id  = '.$this->database->Quote($fieldid).' LIMIT 1';
			$this->database->setQuery($query);
			$element = $this->database->loadObject();
		}else{
			$this->database->setQuery('SELECT * FROM #__j2store_field WHERE field_table='.$this->database->Quote($type).' AND field_namekey='.$this->database->Quote($fieldid));
			$element = $this->database->loadObject();
		}
	...
	function &getData($area,$type,$notcoreonly=false){
		...
		if(empty($data[$key])){
			...
			$this->database->setQuery('SELECT * FROM #__j2store_field as a WHERE '.implode(' AND ',$this->where).' '.$filters.' ORDER BY a.`ordering` ASC');
	...
	function get($field_id,$default=null){
		$query = 'SELECT a.* FROM #__j2store_field as a WHERE a.`field_id` = '.intval($field_id).' LIMIT 1';
		$this->database->setQuery($query);

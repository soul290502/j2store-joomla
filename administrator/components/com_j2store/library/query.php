 ...
 * @since 2.5
 */
class J2StoreQueryElement extends JObject
{
 ...
 * @since 2.5
 */
class J2StoreQuery extends JObject
{
	...
	public function select($columns)
	{
		...
		if (is_null($this->_select)) {
			$this->_select = new J2StoreQueryElement('SELECT', $columns);
		} else {
	...
	{
		$this->_type = 'delete';
		$this->_delete = new J2StoreQueryElement('DELETE', array(), '');
		return $this;
	}
	...
	{
		$this->_type = 'insert';
		$this->_insert = new J2StoreQueryElement('INSERT INTO', $tables);
		return $this;
	}
	...
	{
		$this->_type = 'update';
		$this->_update = new J2StoreQueryElement('UPDATE', $tables);
		return $this;
	}
	...
	public function from($tables)
	{
		if (is_null($this->_from)) {
			$this->_from = new J2StoreQueryElement('FROM', $tables);
		} else {
	...
	public function join($type, $conditions)
	{
		...
		$this->_join[] = new J2StoreQueryElement(strtoupper($type) . ' JOIN', $conditions);
	...
	public function set($conditions, $glue=',')
	{
		if (is_null($this->_set)) {
			...
			$this->_set = new J2StoreQueryElement('SET', $conditions, "\n\t$glue ");
		} else {
	...
	public function where($conditions, $glue='AND')
	{
		if (is_null($this->_where)) {
			$glue = strtoupper($glue);
			$this->_where = new J2StoreQueryElement('WHERE', $conditions, "\n\t$glue ");
		} else {
	...
	public function group($columns)
	{
		if (is_null($this->_group)) {
			$this->_group = new J2StoreQueryElement('GROUP BY', $columns);
		} else {
	...
	public function having($conditions, $glue='AND')
	{
		if (is_null($this->_having)) {
			$glue = strtoupper($glue);
			$this->_having = new J2StoreQueryElement('HAVING', $conditions, "\n\t$glue ");
		} else {
	...
	public function order($columns)
	{
		if (is_null($this->_order)) {
			$this->_order = new J2StoreQueryElement('ORDER BY', $columns);
		} else {

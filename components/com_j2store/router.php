...
require_once(JPATH_SITE.'/components/com_j2store/helpers/router.php');
...
function J2StoreBuildRoute( & $query) {
	...
	$view = J2StoreRouterHelper::getAndPop($query, 'view', 'mycart');
	...
	$qoptions = array( 'option' => 'com_j2store', 'view' => $view, 'task' => $task, 'id' => $id );
 ...
 */				// Joomla! will let the menu item naming work its magic
		...
		case 'orders':
			...
			if(empty($Itemid))
			{
				...
				$menu_id = J2StoreRouterHelper::findMenuOrders($qoptions);
		...
		case 'downloads':
			...
			case 'myprofile':
				...
				if($Itemid) {
					...
					$mTask = isset($menu->query['task']) ? $menu->query['task'] : $task;
					// No, we have to find another root
					if( ($mView != 'myprofile') ) $Itemid = null;
				}
...
function J2StoreParseRoute($segments) {
	...
	if(is_null($menu) && count($segments)) {
		...
		if($segments[0] == 'checkout' ) {
			...
			/* if(isset($segments[2])) {
				$vars['orderpayment_type'] = $segments[2];
			}
			if(isset($segments[3])) {
				$vars['paction'] = $segments[3];
			} */
		}

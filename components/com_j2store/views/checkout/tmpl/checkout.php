				...
				if (shipping_address) {
					$.ajax({
						...
						success: function(html) {
							...
							$.ajax({
								url: 'index.php?option=com_j2store&view=checkout&task=shipping_address',
								dataType: 'html',
								success: function(html) {
...
$(document).on('click', '#button-guest-shipping', function() {
	$.ajax({
		url: 'index.php?option=com_j2store&view=checkout&task=guest_shipping_validate',
		type: 'post',
		...
		success: function(json) {
			...
			if (json['redirect']) {
				...
				if (json['error']['shipping']) {
					...
					$(window).scrollTop($('#shipping_error_div .j2error').offset().top);
				}

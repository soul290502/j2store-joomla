...
<div id="j2store-cart-shipping" class="j2store-cart-shipping">
	...
	?>
	<input type="radio" id="shipping_<?php echo $method['element']; ?>" rel="<?php echo addslashes($method['name'])?>" name="shipping_method" <?php echo $checked; ?> onClick="j2storeUpdateShipping('<?php echo addslashes($method['name']); ?>','<?php echo $method['price']; ?>',<?php echo $method['tax']; ?>,<?php echo $method['extra']; ?>, '<?php echo $method['code']; ?>', true );" />
	<label for="shipping_<?php echo $method['element']; ?>" onClick="j2storeUpdateShipping('<?php echo addslashes($method['name']); ?>','<?php echo $method['price']; ?>',<?php echo $method['tax']; ?>,<?php echo $method['extra']; ?>, '<?php echo $method['code']; ?>', true );">
		<?php echo $method['name']; ?> ( <?php echo J2StorePrices::number( $method['total']); ?> )
	</label>
	...
	function j2storeUpdateShipping(name, price, tax, extra, code, combined) {
		(function($) {
			...
			$.ajax({
				url: 'index.php',
				type: 'post',
				data: $('#j2store-cart-form input[type=\'hidden\'], #j2store-cart-form input[type=\'radio\']:checked'),
				dataType: 'json',
				beforeSend: function() {

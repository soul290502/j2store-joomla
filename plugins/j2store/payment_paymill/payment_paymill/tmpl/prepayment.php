...
function j2storePayMillSubmit(button) {
	...
	(function($) {
		...
		$(button).val('<?php echo JText::_('J2STORE_PAYMILL_PROCESSING_PLEASE_WAIT')?>');
	...
	//	} else {
	//		$(button).val('<?php echo JText::_('J2STORE_PAYMILL_ERROR_PROCESSING')?>');
	//	}
        	...
        	(function($) {
				...
				if(token.length == 0) {
					...
					jqXHR.done(function(json) {
						...
						if (json['redirect']) {
							$(button).val('<?php echo JText::_('J2STORE_PAYMILL_COMPLETED_PROCESSING')?>');
